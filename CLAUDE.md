...
當我在對話中貼上我自己挑好的素材，並明確說「幫我做 Anki 卡」時，才直接執行：
...
單獨提供一個英文 headword，或說 `收錄單字` / `學單字`，不走這個直接輸出流程；
改走下方 Vocabulary Intake，先存 Knowledge Base。`整理 anki` 與 `weekly review`
處理 Vocabulary / Expressions / Sentence Patterns active inbox 中尚未獲得 Anki
coverage 的 Knowledge，以及使用者已看過的 Error Corrections。
所有 Anki 流程產生前都必須搜尋整個 `08_ai/anki/`，相同 target 不得重複建卡。

## Vocabulary Intake（單字先進 Knowledge Base）

當我說 `收錄單字 [word]`、`學單字 [word]`、`[word] 做成單字卡`，或在這個 workflow
脈絡中只提供一個英文 headword 時，執行
`08_ai/anki/vocabulary_intake_workflow.md`。

1. 單字必須由使用者選；AI 不自動挑字。
2. 優先讀 `https://www.vocabulary.com/dictionary/<word>`，取得可見的 IPA、詞性、
   word forms 與 meaning。
3. Front 是不出現答案的精簡英英解釋。
4. Back 包含 exact word、IPA、詞性、frequency、兩個有 domain label 的例句與 word
   forms。
5. Frequency 只有來源頁明確顯示或使用者提供時才能寫；否則標 `Not available`。
6. 網站長篇說明與第三方 examples 不整段複製；definition 改寫，例句預設原創。使用者
   提供的例句可保留並註明。
7. 新 note 存到 `10_knowledge_base/vocabulary/<word-slug>.md`，並在
   `09_coach/knowledge_review_queue.md` 建立 Type = `Vocabulary`、Status = `Pending`。
8. Intake 階段不建立 Anki、不移到 archive。只有已確認 Anki 早已存在時，才可直接存
   `vocabulary/archive/`。

建立前必須搜尋 active Vocabulary、`vocabulary/archive/` 與所有既有 Anki，避免重複。

## Error Correction Intake（原句 → 修正版與分析）

當我說 `記錄造句錯誤`、`收錄造句錯誤`、`記錄這個錯誤` 或
`把這個錯誤留到 Anki` 時，執行
`08_ai/anki/error_correction_intake_workflow.md`。

1. Front 完整保留使用者原句，包含原本的拼字、標點與大小寫錯誤。
2. Back 先放一個保留原意的最佳 Corrected Version，再放 AI／老師提供的完整但精簡
   Error Analysis，最後放一個 Key Reminder。Corrected Version 保持英文；
   Error Analysis 與 Key Reminder 一律使用繁體中文。
3. 一個原句只建立一張卡；不要把每個問題拆成 Vocabulary、Expression 或 Sentence
   Pattern cards。
4. 存到
   `10_knowledge_base/error_corrections/YYYY-MM-DD_<short-slug>.md`，queue Type =
   `Error Correction`、Status = `Pending`。
5. Intake 階段不建立 Anki、不封存。直接輸入 `整理 anki` 代表使用者已看過並批准 active
   Error Corrections，才可以產卡與移到 `error_corrections/archive/`。
6. `weekly review` 只能列出 Pending Error Corrections 請使用者檢查，不可直接輸出或封存。
7. Error Correction note 可作為 evidence，但不繞過 Error DB recurrence/confidence policy。
...
`08_ai/anki/weekly/organize_anki_workflow.md`。

這個指令只執行 Knowledge → Anki 整理，不執行完整 `weekly review`。

1. 掃描 `10_knowledge_base/error_corrections/`、`10_knowledge_base/vocabulary/`、
   `10_knowledge_base/expressions/` 與 `10_knowledge_base/sentence_patterns/` 根目錄
   中的 active Markdown notes。
2. 排除 `TEMPLATE.md`、`README.md` 與所有 `archive/` 內容。
3. 搜尋整個 `08_ai/anki/`，相同 target 不重複建卡。
4. 新卡依 `08_ai/anki/weekly/TEMPLATE.md` 產生 English Front/Back，存到
   `08_ai/anki/weekly/knowledge-review_YYYY-MM-DD.md`。
5. 只有在卡片成功寫入，或已驗證相同卡片存在時，才把來源 note 移到：
   - `10_knowledge_base/vocabulary/archive/`
   - `10_knowledge_base/expressions/archive/`
   - `10_knowledge_base/sentence_patterns/archive/`
   - `10_knowledge_base/error_corrections/archive/`
6. 移動後更新 repo-relative links 與 `09_coach/knowledge_review_queue.md`：
   Status = `Anki`、Last Review = 當日、Practice Evidence = Anki 檔案。

Archive 不是刪除；封存 note 仍是 Knowledge Base canonical source，未來萃取與去重必須
搜尋 active folder 和 `archive/`。格式不完整、卡片寫入失敗或 archive path 衝突的 note
留在 active folder，不可強制移動。

直接 `整理 anki` 是 Error Correction 的人工 approval。若 workflow 是由
`weekly review` 間接呼叫，只列出 Pending Error Corrections，不處理它們。
...
建立任何內容前，搜尋整個 `10_knowledge_base/`，包含 Vocabulary、Expressions、
Sentence Patterns 與 Error Corrections 的 `archive/`，並優先：
...
Vocabulary、Expressions、Sentence Patterns 與 Error Corrections 一個項目一個檔案，格式分別以
`10_knowledge_base/vocabulary/TEMPLATE.md`、
`10_knowledge_base/expressions/TEMPLATE.md` 和
`10_knowledge_base/sentence_patterns/TEMPLATE.md`、
`10_knowledge_base/error_corrections/TEMPLATE.md` 為準。除非 Connie 明確要求，
不要保存中文翻譯；重點是可重用英文，不是 lesson summary。
...
每次新增真正全新的 Vocabulary、Expression、Sentence Pattern 或 Error Correction，也要在
`09_coach/knowledge_review_queue.md` 新增一筆 `Pending`；更新或合併既有知識時，
更新同一筆 queue，不建立重複 row。

最後回報更新或新增了哪些條目、存在哪裡、哪些內容未保存及原因，以及是否更新
Error DB。收錄當下不生成 Anki；由 `整理 anki` 或 `weekly review` 統一分流與生成。
...
5. 執行 Knowledge → Anki 每週分流：
   - 執行 `08_ai/anki/weekly/organize_anki_workflow.md`
   - 處理 Vocabulary / Expressions / Sentence Patterns active folders 中的完整 notes
   - Pending Error Corrections 只列出供 review；不在 `weekly review` 自動產卡或封存
   - 生成前搜尋整個 `08_ai/anki/`；已有相同 target 就不重複
   - 將新卡存到 `08_ai/anki/weekly/knowledge-review_YYYY-MM-DD.md`
   - 卡片成功寫入或已驗證存在後，將 source note 移至同類別的 `archive/`
   - 將 queue 標 `Anki`，Knowledge link 指向 archive，Practice Evidence 連到 Anki 檔
   - 格式不完整或無法驗證 Anki coverage 的 note 留在 active folder
...
├── vocabulary/              ← 使用者選字的 active Anki inbox；archive/ 是已產卡 notes
├── expressions/             ← Active Anki inbox；archive/ 是已產卡 canonical notes
├── sentence_patterns/       ← Active Anki inbox；archive/ 是已產卡 canonical notes
├── error_corrections/       ← 原句/修正版/分析；直接整理 anki 才移入 archive
...
layer 負責練習流程。`vocabulary/` 只收使用者明確選擇的單字；固定 chunks 仍放
`expressions/`。
